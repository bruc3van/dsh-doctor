...
test('catalog dependency policy keeps client-store type relationships development-only', () => {
  ...
  assert.equal(manifest.devDependencies['@deepseek-ai/dsh-client-store'], '0.1.5-rc.2')
})
...
test('a separately investigated future target drives dependency checks without changing the known catalog', () => {
  const root = fixture({
    ...
    manifest: {
      peerDependencies: { '@deepseek-ai/dsh-settings': '^0.1.5-rc.3' },
      devDependencies: { '@deepseek-ai/dsh-settings': '0.1.5-rc.2' },
    },
  })
  const options = { targetVersion: '0.1.5-rc.3' }
  const report = analyzeMigration(root, options)
  assert.deepEqual(report.migration.actualTarget, { version: '0.1.5-rc.3', catalogVersion: '0.1.5-rc.2', catalogExact: false })
  ...
  const manifest = JSON.parse(readFileSync(join(root, 'package.json'), 'utf8'))
  assert.equal(manifest.devDependencies['@deepseek-ai/dsh-settings'], '0.1.5-rc.3')
  assert.equal(manifest.devDependencies['@deepseek-ai/dsh-client-store'], '0.1.5-rc.3')
  assert.equal(applied.report.migration.actualTarget.version, '0.1.5-rc.3')
})
...
test('actual target version stays inside the catalog release line', () => {
  ...
  assert.throws(() => analyzeMigration(root, { targetVersion: '0.1.3-alpha.0' }), /outside the catalog's 0\.1\.5 release line/)
  assert.throws(() => analyzeMigration(root, { targetVersion: '0.1.5-alpha.1' }), /predates the catalog target 0\.1\.5-rc\.2/)
  assert.equal(analyzeMigration(root, { targetVersion: '0.1.5' }).migration.actualTarget.version, '0.1.5')
})
...
if (args[0] === '--version') process.stdout.write('0.1.5-rc.2\\n')
...
test('runtime verification rejects a no-op command that only exits successfully', () => {
  ...
  writeFileSync(fakeDsh, "if (process.argv.includes('--version')) process.stdout.write('0.1.5-rc.2\\n')\n")
  const result = verifyMigration(root, { level: 'runtime', yes: true, install: true, dshCommand: fakeDsh })
  assert.equal(result.passed, false)
  assert.equal(result.status, 'artifact-verified')
...
test('the current 0.1.5 catalog is exact while the 0.1.2 catalogs remain available', () => {
  const current = loadMigration('dsh-v0.1.1-rc.2', 'dsh-v0.1.5-rc.2')
  ...
  assert.ok(current.packages.added.includes('@deepseek-ai/dsh-session-format'))
  ...
  const previous = loadMigration('dsh-v0.1.1-rc.2', 'dsh-v0.1.2-rc.1')
  assert.equal(previous.manifest.to.version, '0.1.2-rc.1')
  assert.equal(previous.manifest.to.commit, 'a66e4702047846cdaa10c66c9d3df3951f5ea70d')
  assert.equal(loadMigration('dsh-v0.1.1-rc.2', 'dsh-v0.1.2-alpha.3').manifest.to.version, '0.1.2-alpha.3')
...
test('the catalog flags packages removed after alpha.3 as removed, not as range mismatches', () => {
  const root = fixture({
  ...
  const catalog = loadMigration('dsh-v0.1.1-rc.2', 'dsh-v0.1.5-rc.2')
...
test('CLI lists catalogs and reports migration failures as one JSON document', () => {
  ...
  assert.deepEqual(JSON.parse(result.stdout).migrations.map(item => item.to.ref), ['dsh-v0.1.2-alpha.2', 'dsh-v0.1.2-alpha.3', 'dsh-v0.1.2-rc.1', 'dsh-v0.1.5-rc.2'])
...
test('runtime verification accepts a separately investigated future 0.1.5 target', () => {
  const root = fixture({ source: 'export const ready = true\n', manifest: {
    peerDependencies: {},
    scripts: { build: 'node -e "process.exit(0)"' },
    files: ['src', 'cordis.patch.yml'],
  ...
  writeFileSync(join(root, 'cordis.patch.yml'), '- insert:\n    - id: fixture-plugin\n      name: fixture-plugin\n')
  const fakeDsh = join(root, 'fake-dsh-rc3.mjs')
  writeFileSync(fakeDsh, `import { mkdirSync, writeFileSync } from 'node:fs'
import { join } from 'node:path'
const args = process.argv.slice(2)
...
`)
  const result = verifyMigration(root, { level: 'runtime', yes: true, install: true, targetVersion: '0.1.5-rc.3', dshCommand: fakeDsh })
  assert.equal(result.passed, true)
